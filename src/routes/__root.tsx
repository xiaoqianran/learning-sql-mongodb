...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      {
        title: "SQL & MongoDB 实战学习 · 系统路径",
      },
      {
        name: "description",
        content:
          "SQL & MongoDB 中文交互教程：清晰学习路径、文档地图、讲解+可运行查询+测验、Catppuccin 主题与查询工坊。",
      },
    ],
    links: [
...
const CTP_BOOT =
  "(function(){try{var f=localStorage.getItem('sql-mongo-learn-ctp-flavor');var a=localStorage.getItem('sql-mongo-learn-ctp-accent');var okF=['mocha','macchiato','frappe','latte'];var okA=['green','mauve','blue','lavender','sapphire','teal','peach','pink'];if(okF.indexOf(f)<0)f='mocha';if(okA.indexOf(a)<0)a='sapphire';document.documentElement.setAttribute('data-ctp-flavor',f);document.documentElement.setAttribute('data-ctp-accent',a);}catch(e){document.documentElement.setAttribute('data-ctp-flavor','mocha');document.documentElement.setAttribute('data-ctp-accent','green');}})();";
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
    ...
    for (const tr of orderedTracks()) init[tr] = tr === contTrack || tr === "SQL 入门";
    return init;
  });
