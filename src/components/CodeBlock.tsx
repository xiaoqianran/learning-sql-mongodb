...
function colorLine(line: string) {
  ...
  const parts = line.split(
    /(\b(?:SELECT|FROM|WHERE|JOIN|LEFT|INNER|ON|GROUP|BY|ORDER|LIMIT|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|AND|OR|AS|IN|LIKE|COUNT|SUM|AVG|MIN|MAX|NULL|IS|NOT|ASC|DESC|import|from|const|let|function|return|export|db|find|findOne|aggregate|updateOne|insertOne|deleteMany)\b|\$[a-zA-Z]+|'[^']*'|"[^"]*"|`[^`]*`)/gi,
  );
  return parts.map((p, i) => {
    if (!p) return null;
    if (/^(SELECT|FROM|WHERE|JOIN|LEFT|INNER|ON|GROUP|BY|ORDER|LIMIT|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|AND|OR|AS|IN|LIKE|COUNT|SUM|AVG|MIN|MAX|NULL|IS|NOT|ASC|DESC|import|from|const|let|function|return|export|db|find|findOne|aggregate|updateOne|insertOne|deleteMany)$/i.test(p)) {
      return (
